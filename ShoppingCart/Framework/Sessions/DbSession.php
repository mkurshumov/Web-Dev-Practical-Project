...
class DbSession extends Database implements ISession
{
    ...
    private function _startNewSession() {
        ...
        $this->prepare('INSERT INTO ' . $this->tableName . ' (sess_id,valid_until) VALUES(?,?)',
    ...
    private function _validateSession() {
        if ($this->sessionId) {
            $d = $this->prepare('SELECT * FROM ' . $this->tableName . ' WHERE sess_id=? AND valid_until<=?',
    ...
    public function saveSession() {
        if ($this->sessionId) {
            $this->prepare('UPDATE ' . $this->tableName . ' SET sess_data=?,valid_until=? WHERE sess_id=?',
